...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    return Math.acos(((x1*x2)+(y1*y2))/((Math.sqrt(Math.pow(x1,2)+Math.pow(x1,2)))*(Math.sqrt(Math.pow(y1,2)+Math.pow(y1,2)))));
}
...
function toNumber(value, def) {
    console.log(value);
    if(typeof value==='string'){var num=parseFloat(value);return isNaN(num)?def:num;}
    else if(typeof value==='number' || typeof value==='object')return console.log(value);
    else return def;
}
